...
def get_masked_environment_config():
    ...
    for layer in base_config['dose_response_pars']['deposition_rates_direct']:
        base_config['dose_response_pars']['deposition_rates_direct'][layer] *= mask_reduction
    
    for layer in base_config['dose_response_pars']['deposition_rates_indirect']:
        base_config['dose_response_pars']['deposition_rates_indirect'][layer] *= mask_reduction
...
def get_improved_ventilation_config():
    ...
    for layer in base_config['dose_response_pars']['deposition_rates_indirect']:
        base_config['dose_response_pars']['deposition_rates_indirect'][layer] *= ventilation_reduction
...
def get_social_distancing_config():
    ...
    for layer in base_config['dose_response_pars']['deposition_rates_direct']:
        base_config['dose_response_pars']['deposition_rates_direct'][layer] *= distancing_reduction
...
def validate_dose_response_config(config):
    ...
    required_keys = ['dose_response_pars', 'viral_load_function']
    
    for key in required_keys:
        if key not in config:
            raise ValueError(f"Missing required key: {key}")
    
    dr_pars = config['dose_response_pars']
    required_dr_keys = ['N0', 'infectivity_scale']
    
    for key in required_dr_keys:
        if key not in dr_pars:
            raise ValueError(f"Missing required dose_response_pars key: {key}")
    
    # Validate ranges
    if dr_pars['N0'] <= 0:
        raise ValueError("N0 must be positive")
    
    if dr_pars['infectivity_scale'] <= 0:
...
def print_config_summary(config):
    ...
    dr_pars = config['dose_response_pars']
    print(f"N0 (infectious dose): {dr_pars['N0']}")
    print(f"Infectivity scale: {dr_pars['infectivity_scale']}")
    
    if 'deposition_rates_direct' in dr_pars:
        print("\nDirect deposition rates:")
        for layer, rate in dr_pars['deposition_rates_direct'].items():
            print(f"  {layer}: {rate:.1f}")
    
    if 'deposition_rates_indirect' in dr_pars:
        print("\nIndirect deposition rates:")
        for layer, rate in dr_pars['deposition_rates_indirect'].items():
            print(f"  {layer}: {rate:.1f}")
    
    vl_pars = config['viral_load_function']
    print(f"\nViral load model: {vl_pars.get('type', 'custom function')}")
    if 'peak_load' in vl_pars:
        print(f"Peak load: {vl_pars['peak_load']:.1e}")
    if 'peak_day' in vl_pars:
        print(f"Peak day: {vl_pars['peak_day']}")
